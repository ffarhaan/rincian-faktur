    ...
    <div className="bg-slate-900/90 border border-slate-800 rounded-2xl shadow-xl overflow-hidden flex flex-col">
      ...
      <div className="p-4 border-b border-slate-800 flex flex-wrap items-center justify-between gap-3 bg-slate-950/40">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input
            ...
            className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500 w-40 sm:w-56"
          />
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl text-xs font-semibold flex items-center gap-1.5 border border-slate-700 transition-colors shadow-sm"
          >
            <Download className="w-3.5 h-3.5" />
            Export Excel
          </button>
          <div className="text-xs text-slate-400 font-medium ml-2">
            Total: <span className="font-bold text-slate-100">{total.toLocaleString('id-ID')}</span> baris
          </div>
        </div>
      </div>
            ...
            {loading ? (
              ...
              rows.map((r) => {
                ...
                return (
                  <tr
                    key={r.id}
                    className={`hover:bg-slate-850/60 transition-colors ${isRetur ? 'bg-rose-950/20' : ''}`}
                  >
                    {/* No. Faktur */}
                    <td className="py-2.5 px-3">
                      <button
                        onClick={() => onSelectInvoice(r.nomor_faktur)}
                        className="font-bold text-slate-200 hover:text-indigo-400 flex items-center gap-1.5 transition-colors group text-left"
                      >
                        <FileText className="w-3 h-3 text-slate-500 group-hover:text-indigo-400" />
                        <span>{r.nomor_faktur}</span>
                        {isRetur && (
                          <span className="px-1 py-0.2 text-[9px] font-extrabold bg-rose-950 text-rose-400 border border-rose-800/80 rounded">
                            RETUR
                          </span>
                        )}
                      </button>
                    ...
                    <td className="py-2.5 px-3">
                      {r.no_so ? (
                        <button
                          ...
                          className="font-mono text-[11px] text-amber-300/90 hover:text-amber-300 flex items-center gap-1 transition-colors group"
                        >
                          <ShoppingCart className="w-3 h-3 text-slate-600 group-hover:text-amber-400" />
                          <span>{r.no_so}</span>
                        </button>
                      ) : (
                    ...
                    {/* Tanggal */}
                    <td className="py-2.5 px-3 text-slate-400 whitespace-nowrap">{r.tanggal}</td>

                    {/* Pelanggan */}
                    <td className="py-2.5 px-3">
                      <button
                        onClick={() => onSelectCustomer(r.nama_pelanggan)}
                        className="font-medium text-slate-200 hover:text-sky-400 transition-colors text-left line-clamp-1"
                      >
                        {r.nama_pelanggan}
                      </button>
                    </td>
                    ...
                    <td className="py-2.5 px-3 max-w-xs">
                      <button
                        ...
                        className="font-medium text-slate-300 hover:text-emerald-400 transition-colors text-left line-clamp-1"
                        title={r.nama_barang}
                      >
                        {r.nama_barang}
                      </button>
                    ...
                    {/* Qty */}
                    <td className={`py-2.5 px-3 text-right font-semibold ${isRetur ? 'text-rose-400' : 'text-slate-200'}`}>
                      {r.kuantitas}
                    </td>

                    {/* Satuan */}
                    <td className="py-2.5 px-3 text-slate-400">{r.satuan}</td>
                    ...
                    {/* Total Harga */}
                    <td className={`py-2.5 px-3 text-right font-semibold ${isRetur ? 'text-rose-400' : 'text-emerald-400'}`}>
      ...
      <div className="p-3 border-t border-slate-800 flex items-center justify-between bg-slate-950/60 text-xs">
        <div className="text-slate-400">
          Halaman <span className="font-bold text-slate-200">{page}</span> dari{' '}
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="p-1.5 bg-slate-800 hover:bg-slate-700 disabled:opacity-30 disabled:pointer-events-none rounded-lg text-slate-300 transition-colors"
          >
