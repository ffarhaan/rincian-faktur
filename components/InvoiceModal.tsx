      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-bold text-slate-100">{invoiceNumber}</h2>
                {data?.is_retur && (
              ...
              <p className="text-xs text-slate-400 mt-0.5">Rincian Faktur Penjualan / Retur</p>
            </div>
          </div>
        ...
        {loading ? (
          <div className="p-12 flex flex-col items-center justify-center gap-3 text-slate-400">
            ...
            <span className="text-sm">Memuat rincian faktur...</span>
          </div>
        ) : data ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-slate-950/40 p-4 rounded-xl border border-slate-800/80">
              <div>
                ...
                <button
                  ...
                  className="font-semibold text-slate-200 text-sm hover:text-indigo-400 flex items-center gap-1.5 mt-1 transition-colors"
                >
              ...
              <div>
                <span className="text-xs text-slate-500 block">Nomor Sales Order (SO)</span>
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-1">
                    {data.linked_sos.map((so: string) => (
                      <button
                        ...
                        className="px-2 py-0.5 text-xs font-semibold bg-amber-950/70 text-amber-300 border border-amber-800/60 rounded hover:bg-amber-900/60 transition-colors flex items-center gap-1"
                      >
              ...
              <div>
                ...
                <span className="font-semibold text-slate-200 text-sm flex items-center gap-1.5 mt-1">
                  ...
                  <tbody className="divide-y divide-slate-850">
                    {data.items?.map((it: any, idx: number) => (
                      <tr key={it.id} className="hover:bg-slate-900/60 transition-colors">
                        ...
                        <td className="py-2 px-3 font-mono text-slate-400">{it.kode_barang || '-'}</td>
                        <td className="py-2 px-3">
                          <button
                            onClick={() => onSelectProduct(it.nama_barang)}
                            className="text-left font-medium text-slate-200 hover:text-emerald-400 transition-colors"
                          >
                            {it.nama_barang}
                          </button>
                        </td>
                        <td className={`py-2 px-3 text-right font-semibold ${it.kuantitas < 0 ? 'text-rose-400' : 'text-slate-200'}`}>
                          {it.kuantitas}
                        </td>
                        <td className="py-2 px-3 text-slate-400">{it.satuan}</td>
                        <td className="py-2 px-3 text-right text-slate-300">
                          Rp {(it.harga_satuan || 0).toLocaleString('id-ID')}
                        </td>
                        <td className={`py-2 px-3 text-right font-semibold ${it.total_harga < 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
                          Rp {(it.total_harga || 0).toLocaleString('id-ID')}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot className="bg-slate-950 font-semibold border-t border-slate-800 text-slate-200">
                    <tr>
                      <td colSpan={3} className="py-3 px-3 text-right">Total Keseluruhan:</td>
                      <td className="py-3 px-3 text-right text-indigo-300">{data.total_qty}</td>
                      <td></td>
                      <td></td>
